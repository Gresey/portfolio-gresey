import codepairImage from '../assets/codepair.png';
import dat from '../assets/dat.png';
import dns from '../assets/dns.png';
import healthmate from '../assets/image.png';
import vaachak from '../assets/vaachak.png';
import emf from '../assets/emf.png';


export const Bio = {
  name: "Gresey Patidar",
  roles: [
    "Full Stack Developer",
    "Mobile App Developer",
    "Competitive Programmer",
  ],
  description:"I am an enthusiastic and adaptable individual, committed to embracing new opportunities and challenges. With a strong passion for continuous learning, I strive to consistently deliver exceptional results. My positive outlook and dedication to personal growth allow me to contribute meaningfully and drive success in any endeavor.",
   github: "https://github.com/gresey",
  resume:
    "https://drive.google.com/file/d/1zAGmuBJ_a5C8qywR0RhVoMRVA_6fRGMH/view?usp=sharing",
  linkedin: "https://www.linkedin.com/in/gresey-patidar-31bb1322b/",
  twitter: "https://twitter.com/Gresey",
  
};

export const skills = [
  {
    title: "",
    skills: [
      {
        name: "Flutter",
        image:
          "https://cdn-images-1.medium.com/max/1200/1*5-aoK8IBmXve5whBQM90GA.png",
      },
      {
        name: "React Js",
        image:
          "data:image/svg+xml;base64,",
      },
     
      {
        name: "HTML",
        image: "https://www.w3.org/html/logo/badge/html5-badge-h-solo.png",
      },
      {
        name: "CSS",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/d/d5/CSS3_logo_and_wordmark.svg/1452px-CSS3_logo_and_wordmark.svg.png",
      },
      {
        name: "JavaScript",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6a/JavaScript-logo.png/800px-JavaScript-logo.png",
      },
      {
        name: "Bootstrap",
        image:
          "https://getbootstrap.com/docs/5.3/assets/brand/bootstrap-logo-shadow.png",
      },
      
    
      {
        name: "Node Js",
        image: "https://nodejs.org/static/images/logo.svg",
      },
      {
        name: "Express Js",
        image:
          "data:image/png;base64,",
      },
      {
        name: "MySQL",
        image:
          "https://raw.githubusercontent.com/devicons/devicon/master/icons/mysql/mysql-original-wordmark.svg",
      },
      
      {
        name: "MongoDB",
        image:
          "https://raw.githubusercontent.com/devicons/devicon/master/icons/mongodb/mongodb-original-wordmark.svg",
      },
      {
        name: "Firebase",
        image: "https://www.vectorlogo.zone/logos/firebase/firebase-icon.svg",
      },
      {
        name: "Java Core",
        image:
          "https://raw.githubusercontent.com/devicons/devicon/master/icons/java/java-original.svg",
      },
      {
        name: "Android Studio",
        image:
          "https://developer.android.com/static/studio/images/new-studio-logo-1_1920.png",
      },
      {
        name: "Git",
        image:
          "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR1bctuHVp7CoSYIgexL8-iR5EfQq-E354UnA&s",
      },
      {
        name: "GitHub",
        image:
          "https://github.githubassets.com/images/modules/logos_page/GitHub-Mark.png",
      },
      {
        name: "VS Code",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/9/9a/Visual_Studio_Code_1.35_icon.svg/512px-Visual_Studio_Code_1.35_icon.svg.png?20210804221519",
      },
     
      
     
    ],
  },
  

 
];

export const experiences = [
 
  
 
];

export const education = [
  {
    id: 1,
    img: "https://images.shiksha.com/mediadata/images/1591596321php4y4orS.jpeg",
    school: "Acropolis Institute of Technology and Research,Indore",
    date: "Nov 2021 - June 2025",
    grade: "8.35 CGPA",
    desc: "I am currently pursuing a Bachelor's degree in Computer Science and Information Technology at Acropolis Institute of Technology and Research,Indore. ",
    degree: "Bachelor of Technology - BTech, Computer Science and Information Technology",
  },
  {
    id: 2,
    img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ86_1_Mr8KUCTmFV6sWxmkJv5XMY6mKfZcj_DvbT7MzOlOGt8gkWJFCrV1yll9QU8Eigc&usqp=CAU",
    school: "Shri Kanwartara Public Higher Secondary School,Mandleshwar,Madhya Pradesh",
    date: "April 2020 - Oct 2021",
    grade: "85.2%",
    desc: "I completed my class 12 high school education at Shri Kanwartara Public Higher Secondary School, Mandleshwar",
    degree: "CBSE",
  },
  {
    id: 3,
    img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQRcFzueCiLRjkKNpXW962cdY0txO_05ljzSbCXpNlN5GOgT8tIwHMKl0mbRXHSrwW8NUY&usqp=CAU",
    school: "Himalaya International School,Dhamnod,dist. Dhar",
    date: "Apr 2017 - Apr 2019",
    grade: "92%",
    desc: "I completed my class 10 education at Himalaya International School,Dhamnod",
    degree: "CBSE",
  },
];

export const projects = [
 
  
  {
    id: 0,
    title: "CodePair",
    date: "",
    description:
      "It is a real-time code collaboration platform where multiple users can collaborate by creating and joining rooms using a Room ID. Users can leave comments on specific lines of code, which are displayed in the right-hand sidebar along with the name of the person who made the comment. Additionally, all users within the same room can chat with each other in real-time.",
    image:
    codepairImage,
    tags: [
      "React Js",
      
      "Node Js",
      "Express Js",
      "Socket.io",
    
    ],
    category: "web app",
    github: "https://github.com/Gresey/CodePair",
    webapp: "code-pair-cyan.vercel.app/",
  },
  
  {
    id: 1,
    title: "DNS Server Configuration Tool",

   date: "",
    description:"Sets up a DNS server that allows you to manage DNS records through a web interface. The server supports various DNS record types, including A, CNAME, MX, PTR, TXT, and AAAA. The web interface provides functionalities to add new DNS records, retrieve all stored records, and display them.",
       image:dns,
    tags: ["React Js","Node Js","Sqlite","Native-dns"],
    category: "web app",
    github: "https://github.com/Gresey/DNS-Server-Configuration-Tool.git",
    webapp: "https://dns-server-configuration-tool.vercel.app/",
  },
  {
    id: 4,
    title: "Daily Attendance Tracker",

   date: "",
    description:"System to track and manage attendance on daily basis in organizations. The system allows teachers to mark attendance for each student in their class and view attendance reports for individual students and the entire class. Bunk monitor feature to track the number of classes a student has missed.Role Based Access Control (RBAC) to manage user roles and permissions.",
        image:dat,
      tags: ["HTML","CSS","Javascript","Sql","JWT"],
    category: "web app",
    github: "https://github.com/Gresey/Daily_Attendance_Tracker.git",
    
  },
  {
    id: 2,
    title: "HealthMate",

   date: "",
    description:"A comprehensive fitness tracking application designed to monitor and visualize key health metrics, helping users maintain and improve their health and wellness routines. The app provides a user-friendly interface that offers detailed insights into various aspects of your health, including water intake, sleep quality, workout time, diet, calories burned, and calories consumed. Additionally, HealthMate features a map finder to help users locate nearby gyms , hospitals and medical facilities.",
          image:healthmate,
     tags: ["Flutter","Dart","BloC","Node Js","Express Js","MongoDB","RestFull API"],
    category: "android app",
    github: "https://github.com/Gresey/HealthMate.git",
    
  },
  {
    id: 3,
    title: "EMF Detector ",

   date: "",
    description:"The EMF Detector app is a sophisticated tool designed by flutter to measure and display electromagnetic fields (EMF) in your surroundings.",
           image:emf,
      tags: ["Flutter","Dart","BloC","Node Js","Express Js","MongoDB","RestFull API"],
    category: "android app",
    github: "https://github.com/Gresey/EMF-Detector-App.git",
    
  },
  {
    id: 4,
    title: "Vaachak",

   date: "",
    description:"This mobile app serves as a crucial and feature-rich extension of a AAC device, enhancing its functionality and providing a user-friendly interface. At the login screen users input the IP address of the Vaachak device. Once logged in, the home screen becomes a personalized hub, allowing users to add and customize signs, tailoring the communication experience to their unique needs.The connect screen enables direct communication with speech therapists. This real-time connection not only supports users in refining their sign language skills but also creates a collaborative space for learning and improvement. The setting screen offers users control over the device's configuration,displaying essential information such as the IP address, battery status, and connection status."
    , image:vaachak,
      tags: ["Flutter","Python"],
    category: "android app",
    github: "https://github.com/Gresey/Vaachak-TecHacks.git",
    
  },
 
    
 
];

export const TimeLineData = [

];
